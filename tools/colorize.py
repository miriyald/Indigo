...
OVERRIDES_FILENAME = "colorize_overrides.tsv"


def export_classification(glyph_plans, output_path, preserve_existing=True):
    """Export auto-classification to TSV. Preserves manual edits if file exists."""
    existing_overrides = {}
    header_comments = []
    ...
    if not header_comments:
        header_comments = [
            "# Auto-generated by: python tools/colorize.py ... --export",
            "# Edit category for specific regions, then re-run colorize.py",
            "# Categories: base, above, disconnected, below, postbase, hole",
    ...
    for plan in glyph_plans:
        ...
        # Build region_idx -> category mapping from classification
        region_cats = {}
        ...
        for idx in sorted(region_cats.keys()):
            region = regions[idx]
            # Use existing override if present, otherwise auto-classified
...
def load_overrides(path):
    """Load TSV overrides. Returns {glyph_name: {region_idx: category_name}}."""
    overrides = {}
    if not path.exists():
        return overrides
...
def main():
    ...
    if args.output:
        ...
        p = Path(args.ttf)
        output_path = p.with_name(p.stem + "-Colorized.ttf")
